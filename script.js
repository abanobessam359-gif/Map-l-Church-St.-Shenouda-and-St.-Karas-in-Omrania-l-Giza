...
document.getElementById("showFamiliesNamesBtn").addEventListener("click", () => {
  let greenNames = [];
  let redNames = [];
  ...
  // عرض النتيجة
  alert(
    `✅ العائلات الصحيحة:\n${greenNames.join(", ") || "لا توجد"}\n\n` +
    `❌ العائلات الخاطئة:\n${redNames.join(", ") || "لا توجد"}`
  );
});

document.getElementById("exportBtn").addEventListener("click", () => {
  let csvContent = "\uFEFF"; // هذا BOM لحل مشكلة العربية
  csvContent += "البيت,اسم العائلة,الحالة\n"; // العناوين

  for (const houseId in houses) {
    houses[houseId].families.forEach(family => {
      const status = family.status || "غير محدد";
      csvContent += `${houseId},${family.name},${status}\n`;
    });
  }
  ...
  link.setAttribute("download", "houses_families.csv");
